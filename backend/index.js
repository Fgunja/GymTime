...
app.post("/pretplata", (req, res) => {
  ...
  db.query(sqlCjenik, [cjenik_id], (err, rezultat) => {
    ...
    // Provjeri ima li već aktivnu pretplatu
    const sqlProvjera = `
    ...
    db.query(sqlProvjera, [korisnik_id], (err, aktivne) => {
      ...
      db.query(
        ...
        (err) => {
          if (err)
            return res.status(500).json({ message: "Greška kod aktivacije" });
...
// Dohvati pretplatu za određenog korisnika (admin pregled)
