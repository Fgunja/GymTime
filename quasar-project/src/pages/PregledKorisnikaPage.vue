        ...
        <template v-slot:body-cell-pretplata="props">
          <q-td :props="props">
            <q-badge
              ...
              :color="
                props.row._pretplata.status_pretplate === 'aktivna'
                  ? 'positive'
                  : 'warning'
              "
            >
        ...
        <q-card style="min-width: 420px; max-width: 620px; width: 100%">
          ...
          <q-card-section>
            <div class="q-gutter-xs text-body2">
              <div>
                <q-icon name="email" class="q-mr-xs text-grey" />{{
                  odabraniKorisnik?.email
                }}
              </div>
              <div>
                <q-icon name="badge" class="q-mr-xs text-grey" />Uloga:
                {{ odabraniKorisnik?.uloga }}
              </div>
              <div>
                <q-icon
                  name="calendar_today"
          ...
          <q-card-section>
            ...
            <div v-if="loadingRezervacije" class="text-center">
              <q-spinner color="primary" />
            </div>
            <div
  ...
  methods: {
    ...
    async otvoriDetalje(korisnik) {
      ...
      try {
        ...
        this.$q.notify({
          type: "negative",
          message: "Greška kod dohvaćanja rezervacija",
        });
      } finally {
    ...
    async promijeniStatus(pretplata, noviStatus) {
      try {
        ...
        const k = this.korisnici.find(
          (k) => k.korisnik_id === this.odabraniKorisnik.korisnik_id
        );
